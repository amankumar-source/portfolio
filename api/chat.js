...
export default async function handler(req, res) {
  ...
  if (req.method !== "POST") {
    return res.status(405).json({
      ...
      reply: "Only POST requests are accepted at /api/chat.",
    });
  }
  ...
  try {
    ...
    if (isRateLimited(clientIp)) {
      console.warn(`[Vercel Serverless Warning] Rate limit exceeded for IP: ${clientIp}`);
    ...
    // Ensure body is parsed (Vercel Node functions automatically parse JSON bodies into req.body)
    ...
    if (!response.ok) {
      ...
      console.error(`[Groq API Error] Status: ${response.status} | Model: ${modelName} | Error: ${errorText}`);
    ...
    console.error("[Vercel Serverless Exception]:", error.message || error);
